// Code generated by mockery v2.42.1. DO NOT EDIT.

package mocks

import (
	models "project-skbackend/internal/models"

	mock "github.com/stretchr/testify/mock"

	utpagination "project-skbackend/packages/utils/utpagination"

	uuid "github.com/google/uuid"
)

// IMemberRepository is an autogenerated mock type for the IMemberRepository type
type IMemberRepository struct {
	mock.Mock
}

// Create provides a mock function with given fields: m
func (_m *IMemberRepository) Create(m models.Member) (*models.Member, error) {
	ret := _m.Called(m)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 *models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func(models.Member) (*models.Member, error)); ok {
		return rf(m)
	}
	if rf, ok := ret.Get(0).(func(models.Member) *models.Member); ok {
		r0 = rf(m)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func(models.Member) error); ok {
		r1 = rf(m)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Delete provides a mock function with given fields: m
func (_m *IMemberRepository) Delete(m models.Member) error {
	ret := _m.Called(m)

	if len(ret) == 0 {
		panic("no return value specified for Delete")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(models.Member) error); ok {
		r0 = rf(m)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// FindAll provides a mock function with given fields: p
func (_m *IMemberRepository) FindAll(p utpagination.Pagination) (*utpagination.Pagination, error) {
	ret := _m.Called(p)

	if len(ret) == 0 {
		panic("no return value specified for FindAll")
	}

	var r0 *utpagination.Pagination
	var r1 error
	if rf, ok := ret.Get(0).(func(utpagination.Pagination) (*utpagination.Pagination, error)); ok {
		return rf(p)
	}
	if rf, ok := ret.Get(0).(func(utpagination.Pagination) *utpagination.Pagination); ok {
		r0 = rf(p)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*utpagination.Pagination)
		}
	}

	if rf, ok := ret.Get(1).(func(utpagination.Pagination) error); ok {
		r1 = rf(p)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByEmail provides a mock function with given fields: email
func (_m *IMemberRepository) FindByEmail(email string) (*models.Member, error) {
	ret := _m.Called(email)

	if len(ret) == 0 {
		panic("no return value specified for FindByEmail")
	}

	var r0 *models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*models.Member, error)); ok {
		return rf(email)
	}
	if rf, ok := ret.Get(0).(func(string) *models.Member); ok {
		r0 = rf(email)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(email)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByID provides a mock function with given fields: id
func (_m *IMemberRepository) FindByID(id uuid.UUID) (*models.Member, error) {
	ret := _m.Called(id)

	if len(ret) == 0 {
		panic("no return value specified for FindByID")
	}

	var r0 *models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func(uuid.UUID) (*models.Member, error)); ok {
		return rf(id)
	}
	if rf, ok := ret.Get(0).(func(uuid.UUID) *models.Member); ok {
		r0 = rf(id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func(uuid.UUID) error); ok {
		r1 = rf(id)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByUserID provides a mock function with given fields: uid
func (_m *IMemberRepository) FindByUserID(uid uuid.UUID) (*models.Member, error) {
	ret := _m.Called(uid)

	if len(ret) == 0 {
		panic("no return value specified for FindByUserID")
	}

	var r0 *models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func(uuid.UUID) (*models.Member, error)); ok {
		return rf(uid)
	}
	if rf, ok := ret.Get(0).(func(uuid.UUID) *models.Member); ok {
		r0 = rf(uid)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func(uuid.UUID) error); ok {
		r1 = rf(uid)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Read provides a mock function with given fields:
func (_m *IMemberRepository) Read() ([]*models.Member, error) {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for Read")
	}

	var r0 []*models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func() ([]*models.Member, error)); ok {
		return rf()
	}
	if rf, ok := ret.Get(0).(func() []*models.Member); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Update provides a mock function with given fields: m
func (_m *IMemberRepository) Update(m models.Member) (*models.Member, error) {
	ret := _m.Called(m)

	if len(ret) == 0 {
		panic("no return value specified for Update")
	}

	var r0 *models.Member
	var r1 error
	if rf, ok := ret.Get(0).(func(models.Member) (*models.Member, error)); ok {
		return rf(m)
	}
	if rf, ok := ret.Get(0).(func(models.Member) *models.Member); ok {
		r0 = rf(m)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Member)
		}
	}

	if rf, ok := ret.Get(1).(func(models.Member) error); ok {
		r1 = rf(m)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// NewIMemberRepository creates a new instance of IMemberRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewIMemberRepository(t interface {
	mock.TestingT
	Cleanup(func())
}) *IMemberRepository {
	mock := &IMemberRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
